...
def calendar_selection(calendars_list: list) -> int:
    ...
    Arguments:
        calendars_list:
            A list of calendars as created with the Python CalDAV
            library in a "Calendar(url)" format.

    Returns:
        The index number of the user's choice from the calendar list as an
        integer.
    """
...
def server_connect(url: str = server_url, usrname: str = keyring.get_password("priorg-caldav", "priorg"),
    ...
    Arguments:
        url:
            URL of the CalDAV server. If not provided, default to the value
            stored in configuration.py file.
        usr:
            Username to access the CalDAV server. If not provided, defaults to
            the value stored in the local keychain.
        pas:
            Password to access the CalDAV server. If not provided, defaults to
            the value stored in the local keychain.
...
def server_disconnect(server: object) -> None:
    ...
    Arguments:
        server:
            A Python CalDAV server class object
...
def calendar_discovery(server: object) -> list:
    ...
    Arguments:
        server:
            Is an object, as an instance of Python CalDAV server class.
...
def server_todos_lister(calendar_list: list) -> list:
    ...
    Arguments:
        calendar_list:
            A list of calendars in "Calendar(url)" format such as
...
def server_todo_hasher(server_todos_list: list) -> dict:
    ...
    Arguments:
        server_todos_list:
            List of VTODOs on a CalDAV server in T0D0(url)
            format.
...
def ics_files_hasher(path: str) -> dict:
    ...
    Arguments:
        path:
            A string containing the path to local files directory.
    Returns:
        Dictionary containing UID/contents-hash-digest value-pairs.
    """
...
def json_file_reader(path: str, name: str) -> dict:
    ...
    Arguments:
        ...
        name:
            Filename of the JSON file (without the extension).
...
def json_file_writer(path: str, name: str, data) -> None:
    ...
    Arguments:
        path:
            Path to the directory where the JSON file should be saved.
        name:
            Filename without extension.
        data:
            Structured data, such as a dictionary.
...
def file_vtodo_finder(path: str, uid: str) -> list:
    ...
    Parameters:
        path:
            A string containing the path to the directory containing the ICS
            files.
        uid:
            A string containing the uid of the VTODO the function will
            search for.

    Returns:
        A list, defined and indexed as below:
            0: UID string of the VTODO;
            1: Filename string of the file containing the VTODO with the UID;
...
def server_vtodo_finder(server_todos: list, uid: str) -> list:
    ...
    Parameters:
        server_todos:
            A list containing the VTODOs on a CalDAV servre in the format
            similar to what the Python CalDAV library generates.
        uid:
            A string containing the uid of the VTODO the function will search
            for.
...
def file_todo_writer(path: str, filename: str, data: object) -> None:
    ...
    Arguments:
        path:
            A string, containing the path to the file that should be created or
            overwritten.
        filename:
            A string containing the name and extension of the target file.
        data:
            VObject instance, holding the VTODO data that should be written to
            the target file.
...
def server_todo_updater(caldav_item: object, data: object) -> None:
    ...
    Arguments:
        caldav_item:
            A CalDAV library VTODO object, pointing to an existing VTODO item on
            a CalDAV server.
        data:
            VObject instance, holding the VTODO data that should be written to
            the target VTODO on the server.
...
def server_todo_creator(calendar: list, data: object) -> None:
    ...
    Arguments:
        calendar:
            A list item from the list of calendars on a CalDAV server in Python
            CalDAV library discovered calendars format.
        data:
            A VObject instance, containing icalendar data to be created on the
            server.
...
def server_vtodo_eraser(item) -> None:
    ...
    Arguments:
        item:
            A VTODO object instance in the format provided by Python CalDAV
            library.
    Returns:
        None.
    """
...
def file_vtodo_eraser(path, filename) -> None:
    ...
    Arguments:
        path:
            Relative path to the directory that contains the file which is to be
            deleted.
        filename:
            Filename of the file which should be deleted, which should contain
            the extension.
